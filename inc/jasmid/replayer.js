...
function Replayer(midiFile, timeWarp, eventProcessor, bpm) {
	...
	function processEvents() {
		function processNext() {
			...
			let modif = 0;
			if (firstnote && midiEvent.event.subtype.startsWith("noteOn")) {
				modif = 1500;
				firstnote = false;
				midiEvent.event.deltaTime = midiEvent.event.deltaTime + modif;
				midiEvent.ticksToEvent = modif;
			}
			temporal.push([ midiEvent, time + modif]);
	...
	return {
		"getData": function() {
			...
			while (i < temporal.length - 1) {
				let sametime = false;
				if (temporal[i].length >= 3) break;
				if (temporal[i + 1][1] == 0) {
					sametime = true;
				}
				temporal[i].push(sametime);
				i++;
			}
