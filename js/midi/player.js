...
midi.loadMidiFile = function(onsuccess, onprogress, onerror) {
	try {
		midi.replayer = new Replayer(MidiFile(midi.currentData), midi.timeWarp);
...
var startAudio = function(currentTime, fromCache, onsuccess) {
	...
	///
  var modif = 0;
	for (var n = 0; n < length && messages < 20 + modif; n++) {
		var obj = data[n];
      ...
      if (channel.mute) break;
				...
				eventQueue.push({
				    ...
				    interval: scheduleTracking(channelId, note, queuedTime + midi.startDelay, offset - foffset, 144, event.velocity, obj[0].track)
				});
        if (obj[2]) {
          modif++;
        }
			...
			case 'noteOff':
				...
				eventQueue.push({
				    ...
				    interval: scheduleTracking(channelId, note, queuedTime, offset - foffset, 128, 0, obj[0].track)
				});
				break;
			default:
				break;
